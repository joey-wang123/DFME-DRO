...
def train(args, teacher, student, generator, device, optimizer, epoch, scale, perturbnet):
...
def test(args, student = None, generator = None, device = "cuda", test_loader = None, epoch=0):
    ...
    myprint('\n MAZE: {}, Test set {}: Average loss: {:.4f}, Accuracy: {}/{} ({:.4f}%), teacher {}, student {},  noise: False\n'.format(
        args.MAZE, args.dataset, test_loss, correct, len(test_loader.dataset), accuracy, args.model, args.student_model))
...
def test_teacher(args, device, teacher = None, test_loader = None, scale = 1.0, perturbnet = None):
    correct = 0
    with torch.no_grad():
        for i, (data, target) in enumerate(test_loader):
            data, target = data.to(device), target.to(device)
            if args.batch_size == data.size(0):
                    if args.batch_size == data.size(0):
                        image_view = data.view(1, -1, 32, 32)
                        perturbation = perturbnet(image_view)
                        perturbation = perturbation.view(args.batch_size, 3, 32, 32)
                        output = teacher(data + scale*perturbation)
                        pred = output.argmax(dim=1, keepdim=True) # get the index of the max log-probability
                        correct += pred.eq(target.view_as(pred)).sum().item()

        accuracy = 100. * correct / len(test_loader.dataset)

    print('\n new test Teacher - Test set: Accuracy: {}/{} ({:.4f}%)\n'.format(correct, len(test_loader.dataset),accuracy))
...
def main():
    ...
    parser.add_argument('--scale', type=float, default=1.0, help = "Fractional decrease in lr")

    parser.add_argument('--dataset', type=str, default='cifar10', choices=['svhn','cifar10','cifar100'], help='dataset name (default: cifar10)')
    ...
    parser.add_argument('--noisenet-max-eps', default=1.5, type=float)
    ...
    if args.model == 'resnet34_8x':
        ...
        print('using random resnet34_8x network')
        if args.dataset == 'svhn':
            print("Loading SVHN TEACHER")
            args.ckpt = 'checkpoint/teacher/svhn-resnet34_8x.pt'

        load_model = torch.load(args.ckpt, map_location=device)
        teacher.load_state_dict(load_model)
        myprint("Teacher restored from %s"%(args.ckpt)) 
    else:
        teacher = get_classifier(args, args.model, pretrained=True, num_classes=args.num_classes)
    
    args.batch_size = 64
    perturbnet_batch_size = int(args.batch_size)
    perturbnet = Res2Net_weight(epsilon=args.noisenet_max_eps, hidden_planes=2, batch_size=perturbnet_batch_size).train().to(device)
    ...
    acc = test_teacher(args, device, teacher = teacher, test_loader = test_loader, scale = args.scale,  perturbnet = perturbnet)
    ...
    for epoch in range(1, number_epochs + 1):
        ...
        train(args, teacher=teacher, student=student, generator=generator, device=device, optimizer=[optimizer_S, optimizer_G], epoch=epoch, scale = args.scale, perturbnet = perturbnet)
