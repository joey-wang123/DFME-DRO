...
def estimate_gradient_objective(args, victim_model, clone_model, x, epsilon = 1e-7, m = 5, verb=False, num_classes=10, device = "cpu", pre_x=False):
    ...
    with torch.no_grad():
        ...
        for i in (range(N * m // max_number_points + 1)): 
            ...
            pred_victim_pts1 = victim_model(pts[:args.batch_size]).detach()
            pred_victim_pts2 = victim_model(pts[args.batch_size:]).detach()
            pred_victim_pts = torch.cat([pred_victim_pts1, pred_victim_pts2])
        ...
        # print('pred_clone.size()', pred_clone.size())
        # print('pred_victim.size()', pred_victim.size())
        # Compute loss
        if args.loss == "kl":
