...
class Bottle2neck(nn.Module):
    def __init__(self, inplanes, planes, stride=1, hidden_planes=9, scale = 1, batch_size=5):
        ...
        new = 1.0
        self.conv1 = noise_Conv2d(inplanes * batch_size, width*scale, kernel_size=1, bias=False, groups=batch_size)
        ...
        for i in range(self.nums):
            ...
            convs.append(noise_Conv2d(width, width, kernel_size=K, stride = stride, padding=P, dilation=D, bias=True, groups=batch_size))
            bns.append(nn.BatchNorm2d(width))
        self.convs = nn.ModuleList(convs)
        self.bns = nn.ModuleList(bns)

        self.conv3 = noise_Conv2d(width*scale, planes * batch_size, kernel_size=1, bias=False, groups=batch_size)
