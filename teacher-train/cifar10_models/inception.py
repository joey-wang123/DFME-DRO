...
import os
from .noise_layer import noise_Conv2d, noise_Linear, data_dependent_Conv2d
...
class Inception3(nn.Module):
    ...
    def __init__(self, args, num_classes=10, aux_logits=False, transform_input=False):
        ...
        self.perturbnet = Res2Net(epsilon=0.50, hidden_planes=2, batch_size=perturbnet_batch_size).train().cuda()
...
#             if isinstance(m, noise_Conv2d) or isinstance(m, nn.Linear):
...
#                 nn.init.constant_(m.bias, 0)
    ...
    def forward(self, x):
        ...
        self.perturbnet.reload_parameters()
        self.perturbnet.set_epsilon(random.uniform(noisenet_max_eps / 2.0, noisenet_max_eps))

        new_fake = x.view(1, -1, 32, 32)
        fake_noise = self.perturbnet(new_fake)
...
class BasicConv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, **kwargs):
        ...
        self.conv = noise_Conv2d(in_channels, out_channels, bias=False, **kwargs)
