...
def get_classifier(args, classifier, pretrained=True, num_classes=10):
    ...
    #     return resnet50(pretrained=pretrained, num_classes=num_classes)
        ...
        return Inception3(args, num_classes=num_classes)
        ...
        return net
    elif classifier == "kt-wrn-28-2":
        print('using the classifer', classifier)
        net = WideResNetKT(args, depth=28, num_classes=num_classes, widen_factor=2, dropRate=0.0)
        if pretrained:
        ...
        return network.resnet_8x_standard.ResNet50_8x(num_classes=num_classes)
    elif classifier == "resnet34_8x":
        if pretrained:
            raise ValueError("Cannot load pretrained resnet34_8x from here")
        return network.resnet_8x_standard.ResNet34_8x(num_classes=num_classes)
    elif classifier == "resnet18_8x":
        if pretrained:
            raise ValueError("Cannot load pretrained resnet18_8x from here")
        print('loading resnet18_8X')
        return network.resnet_8x_standard.ResNet18_8x(num_classes=num_classes)
